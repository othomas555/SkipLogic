...
async function geocodeAddress(address) {
  ...
  if (!apiKey) {
    console.warn("jobs/book-custom geocode skipped: GOOGLE_MAPS_API_KEY missing");
    return null;
  }
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn("jobs/book-custom geocode http failed:", resp.status);
      return null;
    }

    if (data.status !== "OK" || !Array.isArray(data.results) || !data.results.length) {
      console.warn("jobs/book-custom geocode failed:", data.status || "UNKNOWN");
      return null;
    }

    const loc = data.results[0]?.geometry?.location;
    if (!loc || typeof loc.lat !== "number" || typeof loc.lng !== "number") {
      console.warn("jobs/book-custom geocode missing geometry.location");
      return null;
    }
    ...
    console.warn("jobs/book-custom geocode unexpected:", err?.message || err);
    return null;
  }
}
  ...
  try {
    ...
    if (activeJobsLimit != null) {
      const { count, error: cntErr } = await supabase
        ...
        .in("job_status", ["booked", "delivered", "awaiting_collection"]);
    ...
    const custom_skip_description = asText(body.custom_skip_description);
    ...
    if (!custom_skip_description) {
      return res.status(400).json({ ok: false, error: "custom_skip_description is required" });
    }

    const { data: customSkipType, error: customSkipErr } = await supabase
      .from("skip_types")
      .select("id, name")
      .eq("subscriber_id", subscriberId)
      .ilike("name", "Custom")
    ...
    if (customSkipErr) {
      return res.status(500).json({
        ok: false,
        error: "Could not load Custom skip type",
        details: customSkipErr.message,
      });
    }

    if (!customSkipType?.id) {
      return res.status(400).json({
        ok: false,
        error: "Custom skip type not found. Please add a skip type called Custom first.",
      });
    }
    ...
    if (allocErr || !allocatedJobNumber) {
      return res.status(500).json({
        ok: false,
        error: "Could not allocate a job number",
        details: allocErr?.message || null,
      });
    }
    ...
    const insertPayload = {
      ...
      customer_id,
      skip_type_id: customSkipType.id,
    ...
    if (insertError || !job) {
      console.error("jobs/book-custom insert error:", insertError);
      return res.status(400).json({
        ok: false,
        error: insertError?.message || "Could not create custom job",
    ...
    try {
      ...
      if (geocode) {
        ...
        if (geoUpdateErr) {
          console.warn("jobs/book-custom geocode update failed:", geoUpdateErr.message);
        } else {
      ...
      console.warn("jobs/book-custom geocode block failed:", e?.message || e);
    }
    ...
    const { error: eventError } = await supabase.rpc("create_job_event", {
      ...
      _scheduled_at: null,
      _completed_at: null,
      _notes: "Initial custom delivery booked",
    });
    ...
    return res.status(200).json({
      ok: true,
      job,
      custom_skip_type: customSkipType,
      invoice,
      geocode: geocode
        ? {
            ok: true,
    ...
    console.error("jobs/book-custom unexpected error:", err);
    return res.status(500).json({
      ok: false,
