...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (issueErr) {
      return res.status(500).json({ ok: false, error: issueErr.message || "Failed to record issue" });
    }
    ...
    if (unableToComplete) {
      ...
      if (updateErr) {
        return res.status(500).json({ ok: false, error: updateErr.message || "Failed to update job status" });
      }
    }
