...
// - xeroRequest(): calls Xero Accounting API with access token + tenant id
// - resolveXeroContactIdByAccountNumber(): deterministic contact lookup using customers.account_code
//   - if missing: creates a contact with AccountNumber set
//   - if duplicates: throws (do not guess)
...
function escapeForXeroWhere(str) {
  // Xero "where" strings are quoted with double quotes. Escape double quotes.
...
export async function xeroRequest({ accessToken, tenantId, path, method = "GET", body = null }) {
  ...
  if (!res.ok) {
    ...
    const err = new Error(`Xero request failed: ${res.status} ${res.statusText} – ${detail}`);
    // Attach parsed response (if any) for server logs
    err.xero = json;
    throw err;
  }
...
export function periodYmFromJob(job) {
  // Locked: period is based on scheduled_date month; fallback to current UTC month if null.
...
export async function resolveXeroContactIdByAccountNumber({
  accessToken,
  tenantId,
  customer,
}) {
  ...
  if (contacts.length === 1 && contacts[0]?.ContactID) {
...
export async function createOrUpdateDraftMonthlyInvoice({
  accessToken,
  tenantId,
  contactId,
  reference,
  ...
  // Create/update happens outside; this function expects to be called with the target InvoiceID
  // OR used for "create new draft invoice".
  const payload = {
    Invoices: [
      {
        Type: "ACCREC",
...
export async function updateExistingInvoiceLines({
  accessToken,
  tenantId,
  invoiceId,
  contactId,
