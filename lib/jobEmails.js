...
import { getValidXeroClient } from "./xeroOAuth";
...
async function fetchXeroInvoicePdfAttachment({ subscriberId, invoiceId, invoiceNumber, jobNumber }) {
  const safeInvoiceId = asText(invoiceId);
  if (!subscriberId || !safeInvoiceId) return null;

  const xc = await getValidXeroClient(subscriberId);
  if (!xc?.tenantId || !xc?.accessToken) throw new Error("Xero connected but no organisation selected");

  const resp = await fetch(`${XERO_API_BASE}/Invoices/${encodeURIComponent(safeInvoiceId)}`, {
    method: "GET",
    headers: {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Xero invoice PDF request failed: ${resp.status} ${resp.statusText} – ${text}`);
  }

  const filenameBase = asText(invoiceNumber) || asText(jobNumber) || "invoice";
...
export async function sendJobEmail({
  ...
  if (job.skip_type_id) {
    const { data: skipType, error: skipTypeErr } = await supabase
      .from("skip_types")
      .select("id, name")
      .eq("subscriber_id", subscriberId)
    ...
    skipTypeName = asText(skipType?.name);
  }

  if (!skipTypeName && job.custom_skip_description) {
    skipTypeName = asText(job.custom_skip_description);
  }
  ...
  const invoiceNumber = asText(job.xero_invoice_number);
  const invoiceId = asText(job.xero_invoice_id);
  ...
  const tags = {
    ...
    job_ref: jobNumber,
    booking_ref: jobNumber,
    booking_reference: jobNumber,
    booking_number: jobNumber,

    skip_type: skipTypeName,
    skip_type_name: skipTypeName,
    skip: skipTypeName,
    custom_skip_description: asText(job.custom_skip_description),
  ...
  if (invoiceId && (templateKey === "booking_confirmed" || templateKey === "custom_skip_confirmed")) {
    try {
      const invoiceAttachment = await fetchXeroInvoicePdfAttachment({
        subscriberId,
        invoiceId,
        invoiceNumber,
        jobNumber,
      ...
      if (invoiceAttachment) {
        emailAttachments.push(invoiceAttachment);
        invoicePdfAttached = true;
      }
    } catch (err) {
      invoicePdfError = String(err?.message || err);
      console.error("Failed to attach Xero invoice PDF:", err);
    }
  }
  ...
  try {
    ...
    return {
      ...
      to_email: toEmail,
      template_key: templateKey,
      invoice_pdf_attached: invoicePdfAttached,
      invoice_pdf_error: invoicePdfError || null,
      attachments: normaliseAttachments(emailAttachments).map((a) => a.filename),
    };
  } catch (err) {
    if (outboxId) {
      await supabase
