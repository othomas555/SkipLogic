...
async function sendViaResend({ from, to, subject, html, replyTo, bcc }) {
...
export async function sendJobEmail({ subscriberId, jobId, templateKey, extraTags = {} }) {
  ...
  const collectedDate = job.collection_actual_date || extraTags.collected_date || extraTags.collection_date || "";
  ...
  try {
    ...
    return { ok: true, sent: true, to_email: toEmail, template_key: templateKey };
  } catch (err) {
    if (outboxId) {
      await supabase
        .from("email_outbox")
